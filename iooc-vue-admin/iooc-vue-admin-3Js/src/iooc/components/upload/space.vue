<template>
	<div class="cl-upload-space__wrap">
		...
		<!-- 弹框 -->
		<cl-dialog v-model="visible" v-bind="props" :op-list="['close']">
...
import { last, isEmpty } from "@/iooc/utils";
...
export default {
	...
	components: {
		fileItem: {
			...
			methods: {
				onSelect() {
					this.parent.selectFile(this.modelValue);
				},

				onContextMenu(e) {
					this.parent.openFileContextMenu(e, this.modelValue);
					e.stopPropagation();
					e.preventDefault();
				}
			},
			...
			render() {
				...
				const { url, type, selected } = this.modelValue;
